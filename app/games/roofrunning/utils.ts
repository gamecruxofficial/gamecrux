export type SquareIcon = "triangle" | "cross" | "circle";
export type SquareValue = SquareIcon | "empty";

export const squareIcons: SquareIcon[] = ["triangle", "cross", "circle"];

export const getRandomIcon = (): SquareIcon => {
  return squareIcons[Math.floor(Math.random() * squareIcons.length)];
};
...
export const getCluster = (
  ...
  const icon = board[index];
  const cluster: number[] = [];
  const visited: boolean[] = new Array(rows * columns).fill(false);

  // Helper function to recursively find adjacent squares of the same icon using a depth-first search
  function dfs(currentIndex: number) {
    ...
    for (const adjIndex of adjacentIndices) {
      // Check if adjacent square is within bounds and has the same icon
      if (
        adjIndex >= 0 &&
